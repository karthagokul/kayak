...
extern structlog GologgerConfiguration;
...
public:
    ...
    LOG(typelog type,int debug_depth=0) {
        msglevel = type;
        depth=debug_depth;
        if(GologgerConfiguration.headers) {
            ...
            if(GologgerConfiguration.timestamp)
            {
    ...
    LOG &operator<<(const T &msg) {
        if(msglevel & GologgerConfiguration.level) {
            if(GologgerConfiguration.debug_depth>=depth)
            {
                cout << msg;
                opened = true;
            }
